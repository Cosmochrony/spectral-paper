\subsection{Strong-Field Constitutive Coupling Near a Schwarzschild Black Hole}
  ...
  \paragraph{Purpose and epistemic status.}
    ...
    It provides a coarse-grained and purely constitutive parametrization of how
    collective structural constraints within $\chi$ suppress relaxation transport in
    regimes where an emergent geometric description becomes applicable.
    ...
    The purpose of the present appendix is not to modify or extend general relativity,
    but to construct a \emph{self-consistent strong-field constitutive description}
    within the Cosmochrony framework, suitable for characterizing the approach to an
    effective horizon without introducing geometric singularities.
  ...
  \paragraph{Operational time-dilation factor.}
    ...
    excitations.
    This quantity is an operational descriptor within the effective geometric regime
    and does not correspond to a fundamental lapse function of an underlying spacetime.

    In the weak-field limit, $N(r)$ reduces to
    \(N \simeq 1 + \Phi/c^2\),
  ...
  \paragraph{Matching to Schwarzschild phenomenology.}
    ...
    We therefore adopt, \emph{purely as an effective and auxiliary descriptor},
    \begin{equation}
      ds^2
      =
      -f(r)c^2 dt^2
    ...
    This metric has no ontological status within Cosmochrony and serves only as a
    compact parametrization of the effective exterior regime.
    ...
    local relaxation and defines an \emph{effective horizon} within the projected
    description.
  ...
  \paragraph{From relaxation slowdown to conductivity.}
    ...
    \end{equation}
    This identification is neither unique nor fundamental.
    ...
    and (iii) remains monotone and bounded throughout the exterior domain.
    ...
    \end{equation}
    This expression should be read strictly as an \emph{effective constitutive law}
    valid only within the emergent geometric regime.

  \paragraph{Implied structural variation of \texorpdfstring{$\chi$}{χ}.}
    ...
    This divergence must not be interpreted as a physical singularity of the $\chi$
    substrate.
    It indicates that the coarse-grained structural descriptor $\Delta\chi$ ceases to
    remain small and that the hydrodynamic and geometric parametrization is pushed
    beyond its domain of validity near the effective horizon.
  ...
  \paragraph{Interpretation: horizons as conductivity zeros.}
    ...
    provide a coherent strong-field constitutive completion of the weak-field Poisson
    description.
