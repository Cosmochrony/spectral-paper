...
\begin{document}
  ...
  \paragraph{Dynamical survival of spectral sectors.}
    The admissibility hierarchy derived in the present work is structural.
    A key open question is whether the same hierarchy is dynamically realised
  ...
  \paragraph{Mode coupling beyond the one-mode sector.}
    The exact DBI envelope derived here applies to single-mode trajectories.
    The next theoretical step is to analyse multi-mode DBI dynamics, where
    different eigenmodes interact through the local square-root constraint.

    In that regime, the key issue is whether the one-mode admissibility
    hierarchy remains stable under modal mixing, or whether nonlinear transfer
    terms generate new collective selection effects.
  ...
  \paragraph{Selection of generating sets.}
    The comparison between $Q_8$ and $2I$ shows that the generating set is not
    a mere technical detail.
    It enters directly into the adjacency spectrum and therefore into the
    admissibility hierarchy.

    A remaining task is to identify structural criteria selecting physically
    distinguished generating sets, especially on larger finite subgroups of
    $\mathrm{SU}(2)$.
    This is necessary to determine whether the observed low-spin-favoured
    hierarchy is robust or generator-dependent in the continuum-oriented limit.
