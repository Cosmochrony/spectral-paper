...
\begin{document}
  ...
  \paragraph{Step 2: non-perturbative modal DBI reduction.}
    The weakly non-linear reduction has now been obtained explicitly in
    Section~\ref{subsec:weakly-nonlinear-dbi}.
    The next task is to go beyond the expansion in $c_\chi^{-2}$ and derive the
    full amplitude-dependent period from the exact one-mode Lagrangian
    \eqref{eq:exact-mode-lagrangian}, for instance through elliptic-integral methods
    or direct numerical quadrature.
    This will yield the non-perturbative admissibility envelope replacing the
    linear boundary~\eqref{eq:envelope}.
